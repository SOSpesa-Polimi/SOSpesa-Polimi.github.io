	...
	if(isset($_GET['_id'])){
			...
			if($document->onp == $_SESSION['user']
		  	|| $_SESSION['user']=="admin"){
					...
					$result = $manager->executeBulkWrite('siveq.surplus', $bulkC, $writeConcern);

					echo "Executing delete...<br>";

					$bulk = new MongoDB\Driver\BulkWrite;
					$bulk->delete(['_id' => new MongoDB\BSON\ObjectID($_GET['_id'])], ['limit' => 1]);
					$cursor = $manager->executeBulkWrite("siveq.collected", $bulk);
					echo "Moved succesfully<br>";

				} else echo "Permission Denied";	
			}
	} else echo "Parameters not initialized";
?>
</html>